...
- **Front end is assignment-based, not role-based**, so a maker or marketing guide of any role (they're typically Subscribers) can use `[srt_maker_dashboard]` and `[srt_task_view]`. `srt_user_can_access_task()` allows: admins, the assigned maker, the assigned marketing guide, and — only when `srt_restrict_to_maker` is off — Editors and above. That last branch is deliberately not "any logged-in user": the task page is an editable form.
